...
const getBusinessDetails = async (req: express.Request, res: express.Response): Promise<express.Response<never>> => {
	const { businessId }: IBusinessIdRequest = req.params as never;
	if (!businessId) {
		return res.status(400).json({ error: 'Business ID is required.' });
	}
	const detailsResult = await axios.get(`${config.yelp.uri}/businesses/${businessId}`, {
